...
export const PSearchBox: React.FC<PSearchBoxProps> = ({
    ...
    return (
        <div
            className={`psearchbox ${getStateClass()} ${className || ""}`}
            style={{
            ...
            {hasValue && !disabled && (
                <button
                    type="button"
                    onClick={handleClear}
                    className={`psearchbox__clear ${SearchButtonVisibility ? "psearchbox__clear--with-btn" : ""}`}
                    title="清除">

...
export const PLoading = ({
    ...
    return (
        <div className={clsx("flex flex-col items-center justify-center min-w-12.5 min-h-12.5", className)}>
            {/* 镐子动画区域 */}
            <div className="relative w-15 h-11.75">
                {/* 镐子 */}
                {State === "loading" && (
                    <svg
                        className="absolute top-1.5 left-2.5 size-8.75"
                ...
                {/* 成功图标 */}
                {State === "success" && (
                    <svg className="absolute top-2.5 left-2.5 size-10" viewBox="0 0 24 24" fill={Foreground}>
                ...
                {State === "error" && (
                    <svg
                        className="absolute top-2.5 left-2.5 size-10"
                        viewBox="0 0 24 24"
                        fill={Foreground}
                        style={{
                ...
                {/* 底部线条 */}
                <div className="absolute bottom-0 left-0 w-6.25 h-0.5 rounded-full" style={{ backgroundColor: Foreground }} />
            </div>

            {/* 加载文本 */}
            <span className="mt-2.5 text-[16px] font-medium" style={{ color: Foreground }}>
                {Text}
            </span>
        </div>
    );
